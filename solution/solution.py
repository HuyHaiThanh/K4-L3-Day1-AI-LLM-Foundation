...
def compare_models(prompt: str) -> dict:
    ...
    return {
        ...
        "gpt4o_cost_estimate": (len(gpt4o_response.split()) / 0.75) / 1000
        * PRICING_PER_1K_TOKENS["gpt-4o"]["output"],
    }
...
def retry_with_backoff(
    ...
    if max_retries < 0 or base_delay < 0:
        raise ValueError("max_retries and base_delay must be non-negative")
...
def run_assistant(
    ...
    Returns:
        Dict thống kê phiên chat:
            ...
            - "total_cost":   float (USD ước tính cho user/assistant mới;
                               chưa gồm persona và history gửi lại)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        texts = [" ".join(result[key].split()) for key in
                 ("prompt", "gpt4o_response", "mini_response")]
        rows.append([
            *(text if len(text) <= 40 else text[:37] + "..." for text in texts),
